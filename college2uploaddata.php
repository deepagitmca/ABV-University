        ...
        <table width="1000" border="0" cellspacing="0" cellpadding="0" align="center">
         <tbody>
            ...
            <tr bgcolor="#F8F8F8">
   	      ...
   	      <p style="font-size: 24px; text-align: center; font-weight: bold;">Welcome <?php echo  $_SESSION['auname']; ?></p>
          	    <table width="500" border="0" align="center" cellpadding="5" cellspacing="10">
                 <tbody>
                  <tr>
                    <td bgcolor="#909090" style="text-align: center; color: #FDFDFD;"><h2>Jain  college upload data form</h2></td>
                  </tr>
                  <tr>
...
<?php 
	...
	if (isset($_POST["submit"])) 
	{
		...
		if ($_FILES["studentdata"]["size"] > 0)
		{
		 ...
		 {
            $sqlInsert = "INSERT INTO college2student(usn,name) values ('" . $data[0] . "','" . $data[1] . "')";
	...
	if (isset($_POST["submit"])) 
	{
		...
		if ($_FILES["academicdata"]["size"] > 0)
		{
		 ...
		 {
            $sqlInsert1 = "INSERT INTO college2academicdata(usn,name,result) values ('" . $data1[0] . "','" . $data1[1] . "','" . $data1[2] ."')";
		...
		move_uploaded_file($edata_tmp1,"uploads/".$edata1);
		    ...
		    $insertquery = "INSERT INTO college2data(collegecode,collegename,date,studentdata,academicsdata,eventsdata) values('$colcode','$colname','$date1','$sdata1','$adata1','$edata1')";
...
<?php 
	...
	if (isset($_POST["submit"])) 
	{
		...
		if ($_FILES["academicdata"]["size"] > 0)
		{
		 ...
		 {
            $sqlInsert1 = "INSERT INTO college2academicdata(usn,name,result) values ('" . $data1[0] . "','" . $data1[1] . "','" . $data1[2] ."')";
            if($conn1->query($sqlInsert1)===TRUE)
			{
                echo "<script> if(confirm('Uploaded csv successfully')) </script>";
            } 
			else 
		...
		move_uploaded_file($edata_tmp1,"uploads/".$edata1);
			...
			{
				echo "<script> if(confirm('Uploaded successfully')) window.location='college2admindashboard.php';</script>";
			}
			else
			{
				echo "Error : " . $insertquery . "<br>" . $conn1->error;
			}
			
		}
		else
		{
			echo "Error : " , "<br>" . $conn1->error;
		}
		$conn1->close();
?>
